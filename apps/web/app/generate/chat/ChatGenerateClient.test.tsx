...
vi.mock("@/lib/api-client", () => ({
  ...
  getChatThreadResumeState: vi.fn(async (threadId: string) => ({
    success: true,
    resume_state: {
...
describe("ChatGenerateClient", () => {
  ...
  it("rejects completed generation jobs when the final brief is empty", async () => {
    const chatClientModule = (await import("./ChatGenerateClient")) as typeof import("./ChatGenerateClient") & {
      generationJobToChatTurnResponse: (job: unknown, copyGenerationMode?: "suggest_candidates") => unknown;
    };

    expect(() =>
      chatClientModule.generationJobToChatTurnResponse(
        {
          job_id: "job_empty_brief",
          thread_id: "thread_empty_brief",
          status: "done",
          progress: { progress_percent: 100, current_stage: "completed" },
          result_payload: {
            final_image_path: "data/outputs/job_empty_brief/final.png",
            final_brief: {}
          },
          metadata: {},
  ...
  it("opens a pending thread from the authoritative resume job without creating a new job", async () => {
    const api = await import("@/lib/api-client");
    ...
    vi.mocked(api.getChatThreadResumeState).mockResolvedValueOnce({
      ...
      resume_state: {
        ...
        thread_id: "thread_pending_resume",
        resume_job_id: "job_pending_resume",
        final_output_id: null,
        latest_snapshot_id: "snapshot_pending_resume",
        snapshot_kind: "waiting_user_input",
        reason: "job_waiting_user_input",
        current_question: {
          field: "item_or_service",
          question: "홍보할 상품이나 서비스는 무엇인가요?",
          options: [{ id: 1, label: "대표 메뉴", value: "signature_item" }]
    ...
    vi.mocked(api.getChatThreadState).mockResolvedValueOnce({
      success: true,
      snapshot: {
        snapshot_id: "snapshot_pending_resume",
        thread_id: "thread_pending_resume",
        job_id: "job_pending_resume",
        snapshot_version: 1,
        schema_version: 1,
        snapshot_kind: "waiting_user_input",
        state_payload: {
          user_input: "프리미엄 뷰티살롱 홍보 포스터 만들어줘",
          context: {
            business_type: "beauty_salon",
            item_or_service: null,
            promotion_goal: "new_launch"
    ...
    (globalThis as typeof globalThis & { React: typeof React }).React = React;
    searchParamsMock.value = new URLSearchParams("threadId=thread_pending_resume");
    const { ChatGenerateClient } = await import("./ChatGenerateClient");
  ...
  it("stops polling on a waiting job even when the pending interrupt is missing", async () => {
    const api = await import("@/lib/api-client");
    vi.mocked(api.getGenerationJob).mockClear();
    vi.mocked(api.createGenerationJob).mockResolvedValueOnce({
      success: true,
      job: {
        job_id: "job_initial_question_without_interrupt",
        thread_id: "thread_initial_question_without_interrupt",
        status: "running",
        progress: { progress_percent: 24, current_stage: "brief_interpretation" },
        result_payload: null,
        metadata: {},
    ...
    fireEvent.change(screen.getByLabelText("광고 요청 입력"), {
      target: { value: "우리 카페 딸기라떼 광고 만들어줘" }
    });
    fireEvent.click(screen.getByLabelText("요청 보내기"));
  ...
  it("creates the final generation job with the selected GPT-image-2 engine", async () => {
    const api = await import("@/lib/api-client");
    vi.mocked(api.createGenerationJob).mockClear();
    mockInitialAutoPilotBrief(api);
    (globalThis as typeof globalThis & { React: typeof React }).React = React;
    ...
    fireEvent.change(screen.getByLabelText("광고 요청 입력"), {
      target: { value: "우리 카페 딸기라떼 고품질 광고 만들어줘" }
    });
  ...
  it("resumes final generation with custom copy entered from copy candidate selection", async () => {
    const api = await import("@/lib/api-client");
    vi.mocked(api.startChatGeneration).mockClear();
    vi.mocked(api.createGenerationJob).mockClear();
    vi.mocked(api.answerGenerationJob).mockClear();
    ...
    vi.mocked(api.createGenerationJob).mockResolvedValueOnce({
      success: true,
      job: {
        job_id: "generation_job_copy_manual_waiting",
        thread_id: "thread_generation_copy_manual_waiting",
        status: "waiting_user_input",
        progress: {
          progress_percent: 48,
          current_stage: "copy_selection"
    ...
    await waitFor(() =>
      expect(api.answerGenerationJob).toHaveBeenCalledWith("generation_job_copy_manual_waiting", {
        displayText: "직접 쓴 딸기라떼 광고 / 오늘 오후 한정",
        userCustomHeadline: "직접 쓴 딸기라떼 광고",
  ...
  it("keeps restored final generation copy selection on the result path even without source metadata", async () => {
    const api = await import("@/lib/api-client");
    vi.mocked(api.getGenerationJob).mockClear();
    vi.mocked(api.getChatThreadState).mockClear();
    vi.mocked(api.getChatThreadMessages).mockClear();
    vi.mocked(api.answerGenerationJob).mockClear();
    vi.mocked(api.getGenerationJob)
      .mockResolvedValueOnce({
        success: true,
        job: {
          job_id: "generation_job_restored_copy_waiting",
          thread_id: "thread_restored_copy_waiting",
          status: "waiting_user_input",
          progress: {
            progress_percent: 48,
            current_stage: "copy_selection"
    ...
    searchParamsMock.value = new URLSearchParams("jobId=generation_job_restored_copy_waiting&threadId=thread_restored_copy_waiting");
    (globalThis as typeof globalThis & { React: typeof React }).React = React;
    const { ChatGenerateClient } = await import("./ChatGenerateClient");
    ...
    await waitFor(() => expect(screen.getByText("광고 이미지 생성이 완료됐어요")).toBeTruthy());
    expect(navigationMock.replace).not.toHaveBeenCalledWith("/generate/chat/start?threadId=thread_restored_copy_waiting");
  });
  ...
  it("shows a view action for draft studio workspaces with a completed result", async () => {
    const api = await import("@/lib/api-client");
    vi.mocked(api.listChatThreads).mockResolvedValueOnce({
      success: true,
      threads: [
        {
          thread_id: "thread_beauty_done",
          title: "프리미엄 뷰티살롱 포스터",
          status: "draft",
          brand_kit_id: null,
          project_id: null,
          ...
          final_output_id: "output_beauty_done",
          resume_state: {
            action: "view_result",
            thread_id: "thread_beauty_done",
            resume_job_id: "job_beauty_done",
            final_output_id: "output_beauty_done",
            latest_snapshot_id: "snapshot_beauty_done",
    ...
    await waitFor(() => expect(screen.getByText("프리미엄 뷰티살롱 포스터")).toBeTruthy());
  ...
  it("passes the selected GPT-image-2 engine from the photo UI into photo generation start", async () => {
    const api = await import("@/lib/api-client");
    vi.mocked(api.uploadPhotoAsset).mockClear();
    vi.mocked(api.startPhotoGeneration).mockClear();
    vi.mocked(api.createGenerationJob).mockClear();
    ...
    fireEvent.click(screen.getByText("GPT-image-2"));
    fireEvent.change(screen.getByLabelText("사진 광고 요청 입력"), {
      target: { value: "이 사진으로 고품질 신메뉴 광고 만들어줘" }
    });
    fireEvent.click(screen.getByRole("button", { name: /사진 기반 생성 시작/ }));
