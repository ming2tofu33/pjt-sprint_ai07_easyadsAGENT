...
def test_sync_archive_for_job_success(monkeypatch):
    ...
    mock_outputs = {
        ...
        "output_download_url": "https://cdn.example.com/download.png",
        "output_final_image_url": "https://cdn.example.com/final.png",
    }
    ...
    output_repo_mock.get_final_generation_output_for_job.return_value = mock_outputs
    ...
    archive_repo_mock.build_archive_response_row.return_value = {
        **mock_archive_row,
        "workspace_id": "ws1",
        "title": "Cool product",
    ...
    def fake_tx(connection=None):
        conn = connection or object()
        seen_connections.append(conn)
        yield conn

    monkeypatch.setattr("orchestrator.app.archive.service.db_transaction", fake_tx)

    res = sync_archive_for_job(workspace_id="ws1", internal_job_id="job_uuid")

    assert res.title == "Cool product"
    ...
    assert called_kwargs["title"] == "Cool product"
    assert called_kwargs["source"] == "generated"
    assert called_kwargs["output_id"] == "output_uuid"
    assert job_repo_mock.get_generation_job_db_by_id.call_args.kwargs["connection"] is seen_connections[0]
    assert output_repo_mock.get_final_generation_output_for_job.call_args.kwargs["connection"] is seen_connections[0]
    assert called_kwargs["connection"] is seen_connections[0]
    response_kwargs = archive_repo_mock.build_archive_response_row.call_args.kwargs
    assert response_kwargs["output_download_url"] == "https://cdn.example.com/download.png"
    assert response_kwargs["output_final_image_url"] == "https://cdn.example.com/final.png"


def test_sync_archive_for_output_reuses_prefetched_output(monkeypatch):
    mock_job = {
        "id": "job_uuid",
        "public_job_id": "job_public",
        "requested_by": "user1",
    ...
    archive_repo_mock = MagicMock()
    archive_repo_mock.upsert_generated_archive_item_row.return_value = {"public_archive_id": "archive_public", "id": "archive_uuid"}
    archive_repo_mock.build_archive_response_row.return_value = {
        "public_archive_id": "archive_public",
        "title": "Cool product",
        "j_public_job_id": "job_public",
    ...
    monkeypatch.setattr("orchestrator.app.archive.service.job_repo", MagicMock(get_generation_job_db_by_id=MagicMock(return_value=mock_job)))
    output_repo_mock = MagicMock(get_generation_output_by_id=MagicMock(return_value=mock_output))
    output_repo_mock.get_final_generation_output_for_job.side_effect = AssertionError("final output refetch should not run")
    monkeypatch.setattr("orchestrator.app.archive.service.output_repo", output_repo_mock)
    monkeypatch.setattr("orchestrator.app.archive.service.archive_item_repo", archive_repo_mock)
    ...
    res = sync_archive_for_output(workspace_id="ws1", internal_output_id="output_uuid")

    assert res.job_id == "job_public"
    output_repo_mock.get_generation_output_by_id.assert_called_once()
...
def test_archive_list_query_includes_output_payload_for_url_hydration(monkeypatch):
    ...
    assert "o.result_payload ->> 'download_url' as output_download_url" in sql
    assert "o.result_payload ->> 'final_image_url' as output_final_image_url" in sql
