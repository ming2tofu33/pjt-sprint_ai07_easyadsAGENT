...
def test_intake_graph_routes_through_input_evidence_normalizer(monkeypatch):
    ...
    def product_understanding_node(state):
        trace.append("product_understanding")
        return {"product_understanding": {"product_name": "desk lamp"}, "product_understanding_status": "completed"}
    ...
    monkeypatch.setattr(graph_builder, "product_understanding_node", product_understanding_node)
    ...
    assert trace[:4] == ["input", "input_evidence_normalizer", "product_understanding", "validator"]
