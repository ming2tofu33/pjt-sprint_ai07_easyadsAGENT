...
def test_invalid_job_reference_and_request_errors():
    http = client()
    missing_job = http.get("/api/v1/generation-jobs/job_missing")
    assert missing_job.status_code == 404
    assert missing_job.json()["detail"]["error_code"] == "generation_job_not_found"

    missing_template = http.post(
        "/api/v1/generation-jobs",
    ...
    invalid = http.post("/api/v1/generation-jobs", json={"user_input": " "})
