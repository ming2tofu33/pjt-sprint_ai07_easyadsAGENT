...
def test_create_generation_job_defaults_and_lookup():
    ...
    assert job.metadata["user_id"] == "user_1"
    assert get_generation_job(job.job_id) == job


def test_create_generation_job_degrades_mock_immediate_to_queued_only():
    job = create_generation_job(
        GenerationJobCreateRequest(
...
def test_create_generation_job_truncates_user_input_preview():
    user_input = "x" * 200
    job = create_generation_job(GenerationJobCreateRequest(user_input=user_input))

    assert len(job.metadata["user_input_preview"]) <= 120
    assert job.metadata["user_input_preview"].endswith("...")
