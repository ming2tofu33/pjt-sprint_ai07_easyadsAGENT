...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ["여름 시즌 아이스라떼", "지금 주문하기"],
            [_span__test_ocr_gate_final_ad("여름 시즌 아이스라떼"), _span__test_ocr_gate_final_ad("지금 주문하기")],
            id="full-match",
        ),
        pytest.param(
            ["여름 시즌 아이스라떼"],
            [_boxed_span("여름 시즌", 10, 10, 200, 80), _boxed_span("아이스라떼", 210, 10, 400, 80)],
            id="split-spans-match",
        ),
    ],
)
def test_final_expected_copy_pass_cases(expected_text, spans):
    request = OCRValidationRequest(stage="final_ad", image_path="x.png", expected_text=expected_text)
    result = run_ocr_gate(request=request, adapter=FakeOCRAdapter(spans))
    assert_validation_decision(result, "pass")
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            [_span__test_ocr_gate_final_ad("여름 시즌 아이스라떼"), _span__test_ocr_gate_final_ad("SALE")],
            id="unexpected-extra-text",
        ),
        pytest.param(
            [_span__test_ocr_gate_final_ad("SALE")],
            id="unexpected-text-priority-over-missing-copy",
        ),
    ],
)
def test_final_unexpected_text_retries_image(spans):
    request = OCRValidationRequest(stage="final_ad", image_path="x.png", expected_text=["여름 시즌 아이스라떼"])
    result = run_ocr_gate(request=request, adapter=FakeOCRAdapter(spans))
