...
`config._get_env`는 OS env -> `.env` -> default 순으로 읽고, 이 박스의
...
이 플러그인의 autouse fixture는 (1) `.env` 파일 층을 무력화하고 (2) eval 활성화
...
_EVAL_ENV_VARS = (
    ...
    "EASYADS_LOCAL_LLM_PROVIDER",
    "EASYADS_LOCAL_LLM_BASE_URL",
    "EASYADS_LOCAL_LLM_API_KEY",
    "EASYADS_LOCAL_LLM_MODEL",
    "EASYADS_LOCAL_LLM_API_STYLE",
    "EASYADS_LOCAL_LLM_TIMEOUT_SECONDS",
    "EASYADS_LOCAL_LLM_MAX_RETRIES",
...
def _isolate_dotenv(monkeypatch: pytest.MonkeyPatch) -> None:
    # 1) config._get_env가 읽는 .env 파일 층을 무력화
